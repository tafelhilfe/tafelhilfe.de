<template>
    <div class="faqs mb-3" v-if="faqs">
        <b-container>
            <h1 :class="'text-center bb-2 text-tertiary pb-2 mb-2 border-bottom border-'+faqs.borderColor" v-if="faqs.headline !== ''" v-html="faqs.headline"/>
            <b-row class="faqs-tablist" cols="1" v-if="faqs.faq.length" role="tablist">
                <b-col v-for="item in faqs.faq" v-bind:key="item.id">
                    <b-card no-body class="mb-3 rounded-0">
                        <b-card-header class="border-0 pl-0 pr-0" header-tag="header" role="tab" v-on:click="changeIcon(item, faqs)" v-b-toggle="'faq-'+item.id">
