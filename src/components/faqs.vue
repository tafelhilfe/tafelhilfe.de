<template>
    <div class="faqs" v-if="faqs">
        <b-container>
            <h1 class="text-center" v-if="faqs.headline !== ''" v-html="faqs.headline"/>
            <b-row class="faqs-tablist" cols="1" v-if="faqs.faq.length" role="tablist">
                <b-col v-for="item in faqs.faq" v-bind:key="item.id">
                    <b-card no-body>
                        <b-card-header header-tag="header" role="tab" v-on:click="changeIcon(item, faqs)" v-b-toggle="'faq-'+item.id">
                            <h5 class="faqs-card-headline">
                                {{item.header}}
